...
def random_predict(number:int=1) -> int: # Фунция должна получать число 
    ...
    """
    low_number = 1 # Левая граница поиска 
    high_number = 100 # Правая граница поиска
    count_attempts = 0 # Количество попыток 
    number = 0 # Начальное число 
    ...
    while number != predict_number: # Цикл будет выполняться пока условие неравенства истино 
        number = (low_number + high_number) // 2 # Присваиваем начальному числу среднее число списка 
        count_attempts += 1 # Добавляем количество попыток 
        
        if number < predict_number:
            low_number = number + 1 # Если среднее число меньше искомого, отбрасываем левую часть 
        else:
            high_number = number - 1 # Если среднее число больше искомого, отбрасываем правую часть 
    
    return count_attempts # Возвращаем результат количества попыток 
